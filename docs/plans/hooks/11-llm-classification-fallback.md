...
> **Status (2026-04-17):** LANDED. `Classification` is now the four-state
> `{allow, warn, block, unknown}`, `ClassifyBashCommand` returns
> `ClassUnknown` on the default fall-through, and the hook handler
> collapses Unknown to exit 0 in every mode (including block — OQ-5
> compliance). See PR: TBD (add link once merged). The subsections below
> describe the shape of the change as originally specified.
