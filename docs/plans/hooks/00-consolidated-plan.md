...
|---|---|---|---|
| **1a — foundation** | `SessionStart`, `PostToolUse(Edit\|Write)` | T1–T5, T7, T9–T17, T19, T20, T22 landed. T19+T20 green. `hooks doctor` green. Install/uninstall round-trip verified on 3 real `settings.json` samples (empty, existing non-heimdall hooks, existing heimdall hooks from older version). Dogfooded end-to-end on 2026-04-16 (SessionStart + PostToolUse + actor reached `stage=ok`). | Plan approved; §4 reconciliations accepted. |
| **1b — hot path** | `UserPromptSubmit` | 1a verified end-to-end. T4 + T6 landed. Unit tests cover Tier B (Ollama down + model mismatch), cache hit, cache invalidation on `index_version` bump, budget timeout, skip heuristic, disabled env, malformed stdin. `hooks doctor` dry-fire of `user-prompt` green. T21 benchmark **skipped for now** — will be replaced by a same-task with-vs-without-hooks comparison after phase 1b lands, tracked as a follow-up. The original week-long wall-clock soak gate was killed: for a solo local tool, time without usage tests nothing. | 1a gate passed. |
| **2 — session learning** | `Stop` → rolling buffer → `ingest-session` | 1b dogfooded. `Stop` event payload shape confirmed from Claude Code docs (plan 01 OQ §1). Session buffer retention policy confirmed. `ingest-session` failure rate observed from the with-vs-without measurement. | 1b gate passed. |
