...
- Stderr is silent by default. The hook wrapper redirects to `/dev/null`. Real errors go to a log file at **`${XDG_STATE_HOME:-~/.local/state}/heimdall/hooks.log`** (failure-modes 04 §6) with a 5 MB size cap and one rotation generation (`hooks.log.1`). **Stderr must never leak file paths or stack traces** — enforced in `docs/reviews/code-review-context.md`.
